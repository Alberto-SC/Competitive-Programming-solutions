...
do
    ...
    number1=$(cat original_output )
    number2=$(cat brute_output )
    echo "$number1 $number2"
  
    # diff --tabsize=1 -F --label --side-by-side --ignore-space-change original_output brute_output > dont_show_on_terminal; then
