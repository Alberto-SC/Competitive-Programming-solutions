...
int ts(int l ,int r,vector<point> &p,vector<point> &polygon){
    while(l<r) {
        int mid = (l+r)>>1;
        if(area(p,polygon,mid)>area(p,polygon,mid+1)) 
            r = mid;
        else 
            l = mid+1;
    }
    // cout<<"TS "<<r<<endl;
    if(area(p,polygon,l)> area(p,polygon,r))return l%polygon.size();
    else 
        return r%polygon.size();
}
