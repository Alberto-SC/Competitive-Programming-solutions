...
pair<ll, ll> get_interval(ll x, ll y) {
    ll l = 2 * y + (-x);
    ll r = 2 * y - (-x);
    if (l > r) {
        swap(l, r);
...
int main() {
    ...
    int n, q;
    cin >> n >> q;
    typedef IntervalTree<ll, int> itree;
    itree::interval_vector intervals;
    for (int i = 1; i <= n; ++i) {
        ll x, y;
        cin >> x >> y;
        pair<ll, ll> cur = get_interval(x, y);
        intervals.push_back(itree::interval(cur.first, cur.second, i));
    }
    itree tree(move(intervals));
    ll p = 0;
    while (q--) {
        ll a, b;
        cin >> a >> b;
        ...
        ll y = (p + b) % MOD;
        pair<ll, ll> cur = get_interval(x, y);
        itree::interval_vector sol = tree.findOverlapping(cur.first, cur.second);
        vector<int> ids;
        for (auto& i : sol) {
            ids.push_back(i.value);
        }
        sort(ids.begin(), ids.end());
        p = 0;
        ll magic = 1;
        for (ll j = 0; j < (int) ids.size(); ++j) {
            p += (ids[j] * magic) % MOD;
            if (p >= MOD) {
                p -= MOD;
            }
