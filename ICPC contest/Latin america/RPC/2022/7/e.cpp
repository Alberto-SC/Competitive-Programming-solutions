...
ld area(vector<point> &p,vector<point> &polygon,int idx){
    vector<point> nw = p;
    nw.push_back(polygon[idx%polygon.size()]);
...
point rotate_point(point center,float angle,point p){
  ld s = sin(angle);
  ld c = cos(angle);
  p.x -= center.x;
  p.y -= center.y;
...
ld check(ld angle,vector<point> &P ,point door,point first){
    ld A = area(P);
    point rot = rotate_point(door,angle*pi/180,first);
    vector<point> NP = cutPolygon(P,door,rot-door);
    ld A2 = area(NP);
    ld A3 = A-A2;
    return A2-A3;
}
...
int main(){__
    ...
    point door = P[0];
    point az = point(100000000,door.y);

    ld L = atan2(P[n-1].y - door.y, P[n-1].x - door.x)*180/pi;
    ld R = atan2(P[1].y - door.y, P[1].x - door.x)*180/pi;
    cout<<signed_angle(az-door,P[1]-door)<<endl;
    cout<<signed_angle(az-door,P[n-1]-door)<<endl;
    cout<<L<<" "<<R<<endl;
    for(int i=0; i<200; i++) {
        ld m = (L+R)/2;
        if(geq(check(m,P,door,az),0.0)) R = m;
        else L = m;
    }

    cout<<fixed<<setprecision(6);
    point rot = rotate_point(door,R*pi/180,az);
    for(int i = 1; i < n-1; ++i){
		if(intersectLineSegmentInfo(door, rot-door, P[i], P[(i+1)%n]) == 1){
			point p = intersectLines(door, rot-door, P[i], P[(i+1)%n] - P[i]);
            cout<<p.x<<" "<<p.y<<endl;
            break;
		}
	}
