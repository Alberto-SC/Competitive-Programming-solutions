...
int greedy(const Input &input) {
	// sort the proofs on length; iterate over them until a suitable one is found;
	...
	sort(proofs.begin(), proofs.end(),
	     [](const PL &l, const PL &r) { return l.first.length < r.first.length; });
