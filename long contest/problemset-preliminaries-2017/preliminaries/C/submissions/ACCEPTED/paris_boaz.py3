...
class edge:
    ...
    def __str__(self):
        return "[{} -> {}]({}; {})".format(self.v1, self.v2, self.id, self.c)
...
def print_result(gr):
    ...
    while True:
        for e in v_current.edges:
            if ((v_current.parent and e != v_current.parent_edge) or not v_current.parent) and e != v_previous.parent_edge and e not in result:
                result.add(e)
    ...
    if len(result) == 0: print("none")
        ...
        result = sorted(list(result), key = lambda e: e.id)
        first = True
        for e in result:
            if first:
                print("{}".format(e.id), end="")
                first = False
            else: print(" {}".format(e.id), end="")
        print("")

def breadth_first(gr, edges_in_tree):
    destination = len(gr.vertices) - 1
