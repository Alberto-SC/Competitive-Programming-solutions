...
pair<ll, ll> merge(pair<ll, ll> lhs, pair<ll, ll> rhs) {
    ...
    if (lhs.second > rhs.second) {
        const auto diff = lhs.second - rhs.second;
        const auto m = (diff - 1) / rhs.first + 1;
        return merge(lhs, {rhs.first, rhs.second + m * rhs.first});
    } else {
        const auto diff = rhs.second - lhs.second;
        const auto m = (diff - 1) / lhs.first + 1;
        return merge({lhs.first, lhs.second + m * lhs.first}, rhs);
    }
}
