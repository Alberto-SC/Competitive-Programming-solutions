...
pair<double, double> diameterAndWidth(vector<point> & P){
  ...
  for(int i = 0; i < n; ++i){
    while(cruz(i, k) > eps) k = (k+1) % n;
    //pair: (i, k)
    diameter = max(diameter, (P[k] - P[i]).length());
    width = min(width, DistanciaPuntoRecta(P[i], P[(i+1)%n] - P[i], P[k]));
  }
...
int main(){
  ...
  for(int i = 0;i<n;i++){ 
    int in = InterseccionRectas(Linea(p[i][0],p[i][1]),Linea(p[i][2],p[i][3]));
    if(in == 1){
      Punto o = PuntoInterseccion(Linea(p[i][0],p[i][1]),Linea(p[i][2],p[i][3]));
      cout<<"POINT "<<o.x<<" "<<o.y<<endl;
    } 
    else if(in== -1)cout<<"LINE"<<endl;
    else cout<<"NONE"<<endl;
  }
  cout<<"END OF OUTPUT";
